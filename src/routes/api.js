...
router.post('/todos/batch', validateCreateTodoBatch, (req, res) => {
  try {
    const items = req.body.todos.map(({ title, description, priority, tags, due_date }) => ({
    ...
    const createdTodos = todoModel.createMany(items);

    res.status(201).json({ success: true, data: createdTodos });
  } catch (error) {
