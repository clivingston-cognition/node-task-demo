...
router.post('/todos', validateCreateTodo, (req, res) => {
  try {
    const { title, description, priority, tags, due_date, assignee } = req.body;
    const normalizedTitle = normalizeInternationalText(title);
    const todo = todoModel.create({ title: normalizedTitle, description, priority, tags, due_date, assignee });
...
router.put('/todos/:id', validateUpdateTodo, (req, res) => {
  try {
    const { title, description, completed, priority, tags, due_date, assignee } = req.body;
